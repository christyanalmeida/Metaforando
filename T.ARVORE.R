...
for(i in (1:nrow(dbParametros))) {
  ...
  for(vkFolder in 1:10){
    ...
    rtree_fit <- rpart(db_categoria_treino ~ ., 
                       db_train
    )
    #rpart.plot(rtree_fit)
    
    pr <- predict(rtree_fit,db_test,type='vector')
    ...
    parameter.pathFile <- paste('C:/Users/CHRISTYAN/Google Drive/MACKENZIE/MESTRADO/TESE/4.DataStorage/2.DW/MetaIndicadores/Results/DecisionTree/Result.Metaindicadores_PP',PP,'.',WFYean,'_WP',WP,'_WF',WF,'_MF',MF,'_KFolder',vkFolder,'.csv',sep = '')
