...
for(i in (1:nrow(dbParametros))) {
  ...
  for(vkFolder in 1:10){
    ...
    # Classificador Naive
    NBclassfier=naiveBayes(db_categoria_treino ~ ., data=db_train,usekernel=T)
    pr <- predict(NBclassfier, newdata=db_test, type="class")
    
    
    dbNaive <- as.data.frame(pr)
    db_predito <- db_test
    ...
    db_predito$predito <- dbNaive$pr
    
    
    parameter.pathFile <- paste('C:/Users/CHRISTYAN/Google Drive/MACKENZIE/MESTRADO/TESE/4.DataStorage/2.DW/MetaIndicadores/Results/NaiveBayes/Result.Metaindicadores_PP',PP,'.',WFYean,'_WP',WP,'_WF',WF,'_MF',MF,'_KFolder',vkFolder,'.csv',sep = '')
